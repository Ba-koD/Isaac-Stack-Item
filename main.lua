...
function mod:OnUpdate_Habit()
    ...
    for i, info in ipairs(mod.habitPending) do
        local player = info.player
        if player and player:Exists() then
            local slot       = info.slot
            local total0     = info.total0
            local stacks     = info.stacks
...
--   - base game: +1 charge after using an active
--   - this mod: additional stacks add +1 each (so total +count)
-- =================================================================================================================================================================================================================================================
local NINE_VOLT = 116
...
function mod:OnUseItem_NineVolt(itemID, rng, player, flags, slot, varData)
    ...
    -- slot 우선: 콜백에서 주는 slot을 그대로 사용 (없으면 PRIMARY)
    local s = slot or ActiveSlot.SLOT_PRIMARY

    -- 안전장치: 같은 프레임에 중복 등록 방지
    local d = player:GetData()
    ...
    -- 추가 스택당 +1칸 (엔진이 기본 +1을 주므로 우리는 (c-1)만 더함)
    table.insert(mod.nineVoltPending, {
        player = player,
        slot   = s,
        add    = (c - 1),
        frame  = f + 1,   -- 다음 프레임에 적용 (엔진 처리 후)
    })
end
